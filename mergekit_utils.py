...
def _run_command_with_logging(cmd_list: List[str], cwd: str, env: Optional[dict], log_fn: Callable, timeout: Optional[int] = 3600):
    ...
    try:
        ...
        output_thread.join(timeout=10) # Ensure output thread finishes
    ...
    for i, model_source_info in enumerate(merge_config.referenced_models()):
        ...
        try:
            ...
            model_ref = model_source_info.merged(
                cache_dir=lora_merge_cache, huggingface_token=hf_token, trust_remote_code=trust_remote_code_prefetch
            )
            log_fn(f"Getting local path for {model_ref.model}", "DEBUG")
            local_model_path = model_ref.local_path(
                cache_dir=hf_home, huggingface_token=hf_token, trust_remote_code=trust_remote_code_prefetch
            )
...
def _run_merge_operation(
    ...
    output_subdir_name: str, operational_temp_dir_str: str, log_fn: Callable,
    low_cpu_mem: bool = True, read_to_gpu: bool = True, trust_remote_code_config: bool = False
):
    ...
    tmp_env["TMPDIR"] = str(pathlib.Path(abs_operational_temp_dir) / ".cache" / "tmp")
    tmp_env["TRANSFORMERS_CACHE"] = str(pathlib.Path(abs_operational_temp_dir) / ".cache" / "transformers")
    tmp_env["HF_HUB_CACHE"] = tmp_env["HF_HOME"]
    ...
    if current_has_gpu:
        ...
        if low_cpu_mem and "--low-cpu-memory" not in full_cli_list:
    ...
    try:
        _run_command_with_logging(
            full_cli_list, cwd=str(merged_path_workdir), env=tmp_env, log_fn=log_fn, timeout=None
        )
...
def process_model_merge(
    ...
    temp_dir_base: str, log_fn: Callable, trust_remote_code_config: bool = False,
    ...
    final_local_path_str, error_message_str, uploaded_files_list = None, None, None
    try:
        ...
        log_fn(f"Creating operational temp directory in: {temp_dir_base}", "INFO")
        with tempfile.TemporaryDirectory(prefix="mergekit_op_", dir=temp_dir_base) as operational_temp_dir_str:
            log_fn(f"Operational temp directory: {operational_temp_dir_str}", "DEBUG")
            ...
            if local_path_merge_output:
                ...
                try:
                    ...
                    return None, None, f"Error copying to local path: {str(e)}"
            else:
            ...
            repo_url_str, actual_repo_name_used_for_upload = None, repo_name
            if upload_to_hf:
                log_fn(f"Preparing to upload to Hugging Face repo: {repo_name}", "INFO")
                if not effective_hf_token:
                    if repo_name.startswith("mergekit-community/") and community_hf_token_val:
                        effective_hf_token = community_hf_token_val
                ...
                if upload_to_hf and effective_hf_token:
                    try:
                        ...
                        if '/' not in actual_repo_name_used_for_upload:
                            try:
                                ...
                                log_fn(f"Could not determine user namespace for repo name: {e_whoami}. Upload may fail.", "WARNING")
                        repo_url_obj = hf_api_for_upload.create_repo(
                            repo_id=actual_repo_name_used_for_upload, exist_ok=True, private=False
                        )
                        ...
                        hf_api_for_upload.upload_folder(
                            repo_id=actual_repo_name_used_for_upload, folder_path=mergekit_internal_output_dir,
                            commit_message=f"Add merged model: {yaml_config_str[:100]}..."
                        )
